...
const Footer = () => {
  ...
  return (
    <footer className=" bg-back-beige px-10 py-6 rounded-b-md ">
      <div className="h-80">
        <div className="h-full w-full flex flex-row justify-between items-center ">
          <div className=" flex flex-row h-full w-full ">
            <div className="flex flex-col justify-center items-start pl-5 pt-5 w-1/2 gap-2">
            ...
            <div className="flex flex-col justify-center items-center gap-16 w-1/2">
              ...
              <h4 className="text-2xl ">Here are my socials</h4>
              <div className="flex gap-28">
                {datas.map((item) => {
                  return (
                    <a key={item.key} className="hover:scale-125 duration-300" href={item.link} target="_blank"><img className="h-12" src={item.icon} alt={`${item.platform} logo`} /></a>)
                })}
              </div>
            </div>
          </div>
