...
const Footer = () => {
  ...
  return (
    <footer id="contact-section" className="relative shadow-inner flex flex-col bg-light-blue pt-6 h-max">
      ...
      <div className=" flex flex-col md:flex-row justify-between items-center h-full w-full px-16 gap-2">
        <div className="h-fit md:w-2/3 flex flex-col md:flex-row items-center">
          <div className="flex flex-col px-3 md:px-10 justify-center md:items-start bg-footer-blue rounded-md py-10 w-max md:w-2/4 gap-2 ">
            <h3 className="text-3xl font-bold text-white mb-3">Contactez-moi</h3>
            <p className="text-lg text-left">Via email : <a href="mailto:steviggio@gmail.com" type="email">steviggio@gmail.com</a></p>
            <p className="text-left text-lg">Ou en remplissant ce formulaire :</p>
          </div>
          <Form className="flex relative flex-col justify-center items-center w-full" method="post" action="">
            ...
            <input  className="input-props border-2 h-8 w-full md:w-8/12 focus:outline-none rounded-md mb-1" autoComplete="off" type="text" name='email' placeholder="john@doe.fr" />
            {/* <label className="text-xl font-bold text-black" htmlFor="message" >Message</label> */}
            <input  className="input-props h-28 flex relative top-0 w-full md:w-8/12 border-2 focus:outline-none rounded-md overflow-hidden whitespace-pre-wrap" autoComplete="off" type="text" name="message" placeholder="De quoi souhaitez vous discutez ?" />
            <button className="mt-2 w-full md:w-2/4 bg-dark-grey text-white rounded-md hover:scale-110 duration-200 font-bold" type="submit">Envoyez</button>
          </Form>
        </div>
        <div className="flex flex-col justify-center items-center shadow-md bg-footer-blue gap-16 rounded-md w-fit lg:mr-10 lg:p-8">
